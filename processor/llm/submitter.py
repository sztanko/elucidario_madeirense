...
class Submitter:
    ...
    def submit_with_retry(self, group_id, message: str, engine: AIEngine, parser) -> Any:
        try_count = 0
        output = ""
        while try_count < MAX_RETRIES:
            try:
                ...
                try_count += 1
                if try_count == 3:
                    logging.error(f"Group {group_id} Failed after {try_count} attempts")
                    logging.exception(e)
                    logging.info(f"Saving message to {self.fail_dir}")
                    if not os.path.exists(self.fail_dir):
                        os.makedirs(self.fail_dir)
                    file_name = f"{self.fail_dir}/{group_id}_{time.time()}.txt"
                    with open(self.fail_dir, "w") as f:
                        f.write(message)
                else:
            ...
            if len(group) == 1:
                ...
                if article_size > self.threshold:
                    ...
                    result = self.submit_with_retry(
                        group_no, message, self.processor.get_single_article_engine(), self.processor.parse_result
                    )
                yield result
            else:
