...
export function ArticleIndex ({ articleIndex, locale }: ArticleIndexProps) {
  ...
  const letters = _.chain(articleIndex)
    ...
    .toPairs()
    .sort((a, b) => a[0].localeCompare(b[0], locale)) // modified line
    .fromPairs()
    .mapValues(articles => _.sortBy(articles, 'title'))
    .value()

  const letterLinks = _.keys(letters).map(letter => (
    <Link
      href={`#${letter}`}
      key={letter}
  ...
  const letterComponents = _.chain(letters)
    .toPairs()
    .map(([letter, articles]) => (
      <Letter key={letter} letter={letter} articles={articles} />
    ))
    .value()
  // const people
  return (
